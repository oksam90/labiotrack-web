@extends('layouts.app')
@section('title','Activités en temps réel')
@push('styles')
<style>
.pulse-dot {
...
<div class="page-header d-flex align-items-center justify-content-between">
    <div class="d-flex align-items-center gap-2">
        <h4 class="fw-bold mb-0"><i class="bi bi-activity me-2 text-success"></i>Tableau de bord — Activités</h4>
...
<div class="row g-3 mb-4" id="live-stats">
    <div class="col-6 col-md-2">
        <div class="live-stat">
            ...
            <div class="lbl"><i class="bi bi-clipboard-plus me-1"></i>Déclarations aujourd'hui</div>
        </div>
    </div>
    <div class="col-6 col-md-2">
        <div class="live-stat">
            <div class="val" id="stat-collectes">{{ $stats['collectes_today'] }}</div>
            <div class="lbl"><i class="bi bi-truck me-1"></i>Collectes aujourd'hui</div>
        </div>
    </div>
    <div class="col-6 col-md-2">
        <div class="live-stat">
            <div class="val" id="stat-checklists">{{ $stats['checklists_today'] }}</div>
            <div class="lbl"><i class="bi bi-check2-square me-1"></i>Checklists aujourd'hui</div>
        </div>
    </div>
    <div class="col-6 col-md-2">
        <div class="live-stat">
            <div class="val text-danger" id="stat-alertes">{{ $stats['alertes_nonlues'] }}</div>
            <div class="lbl"><i class="bi bi-bell me-1"></i>Alertes non lues</div>
        </div>
    </div>
    <div class="col-6 col-md-2">
        <div class="live-stat">
            <div class="val text-info" id="stat-users">—</div>
            <div class="lbl"><i class="bi bi-people me-1"></i>Utilisateurs actifs (15 min)</div>
        </div>
    </div>
    <div class="col-6 col-md-2">
        <div class="live-stat">
            <div class="val text-secondary">{{ $stats['etablissements'] }}</div>
            <div class="lbl"><i class="bi bi-hospital me-1"></i>Établissements actifs</div>
        </div>
    </div>
</div>
...
<div class="row g-3">
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                <div><i class="bi bi-list-ul me-2 text-success"></i>Flux d'activité — toutes plateformes</div>
    ...
    <div class="col-md-4">
        <div class="card mb-3">
            <div class="card-header"><i class="bi bi-pie-chart me-2 text-success"></i>Répartition par type</div>
            <div class="card-body">
                <canvas id="typeChart" style="max-height:200px;"></canvas>
            </div>
        </div>
        <div class="card">
            <div class="card-header"><i class="bi bi-clock-history me-2 text-success"></i>Refresh automatique</div>
            <div class="card-body">
                <div class="d-flex align-items-center justify-content-between mb-2">
                    <span style="font-size:.85rem;">Intervalle de rafraîchissement</span>
                ...
                <small class="text-muted d-block mt-1" id="next-refresh">Prochain rafraîchissement dans 30s</small>
            </div>
        </div>
    </div>
</div>
...
async function chargerActivites() {
    try {
        const resp = await fetch('{{ route("admin.activites.data") }}');
        ...
        document.getElementById('flux-count').textContent = data.flux.length + ' événements';

        if(data.flux.length === 0) {
            container.innerHTML = '<div class="text-center py-5 text-muted"><i class="bi bi-inbox fs-2 d-block mb-2"></i>Aucune activité récente</div>';
            return;
        }
        ...
        const labels = Object.keys(counts).map(k => ({declaration:'Déclarations',collecte:'Collectes',checklist:'Checklists',alerte:'Alertes',destruction:'Destructions'})[k]||k);
...
function changerIntervalle() {
    ...
    if(intervalMs > 0) {
        ...
        document.getElementById('next-refresh').textContent = 'Rafraîchissement manuel uniquement';
    }
}
