...
<form method="POST" action="{{ route('declarations.update', $declaration->id) }}" id="decl-form" novalidate>@csrf @method('PUT')
    ...
    <div class="table-responsive mb-3">
        <table class="table table-sm align-middle" id="lignes-table">
            ...
            <tbody id="lignes-body">
                @foreach($declaration->lignes as $i => $ligne)
                <tr class="ligne-row">
                    <td>
                        <select name="lignes[{{ $i }}][service_id]" class="form-select form-select-sm sel-service" required>
                            <option value="">{{ __('declarations.line_select_ph') }}</option>
                            @foreach($services as $s)
                            <option value="{{ $s->id }}" {{ $ligne->service_id == $s->id ? 'selected' : '' }}>{{ $s->nom }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
                        <select name="lignes[{{ $i }}][type_contenant_id]" class="form-select form-select-sm sel-contenant" required>
                            <option value="" data-poids="0">{{ __('declarations.line_select_ph') }}</option>
                            @foreach($typeContenants as $tc)
                            <option value="{{ $tc->id }}" data-poids="{{ $tc->poids_moyen_kg }}" {{ $ligne->type_contenant_id == $tc->id ? 'selected' : '' }}>{{ $tc->nom }} (~{{ $tc->poids_moyen_kg }} {{ __('declarations.form_kg_per_unit') }})</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
                        <input type="number" name="lignes[{{ $i }}][nombre_contenants]" class="form-control form-control-sm text-center inp-nombre" value="{{ $ligne->nombre_contenants }}" min="1" max="999" required>
                    </td>
            ...
            <tfoot class="table-light">
                <tr>
                    <th colspan="2" class="text-end">{{ __('declarations.total_containers') }} : <span id="total-nb" class="text-success">0</span></th>
    ...
    <textarea name="notes" class="form-control" rows="2">{{ old('notes', $declaration->notes) }}</textarea></div>
...
(function () {
    ...
    // Repart après les lignes déjà rendues côté serveur.
    let index = body.querySelectorAll('.ligne-row').length;
    ...
    // ── Validation métier côté client (avant PUT) ──────────────
    ...
    // Câble les lignes existantes + calcule le total initial.
    body.querySelectorAll('.ligne-row').forEach(wireRow);
    recalc();
})();
</script>
@endpush
