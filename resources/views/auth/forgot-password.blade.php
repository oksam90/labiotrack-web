...
<head>
    ...
    <title>{{ __('auth_ui.forgot_title') }} — LaBioTrack</title>
    ...
    <div class="row justify-content-center">
        <div class="col-md-5">
            <div class="auth-card mx-auto">
                <h5 class="fw-bold mb-1" style="color:#1B6B3A;">
                    <i class="bi bi-key me-1"></i>{{ __('auth_ui.forgot_title') }}
                </h5>
                <p class="text-muted" style="font-size:.85rem;">{{ __('auth_ui.forgot_intro') }}</p>
                ...
                <form method="POST" action="{{ route('password.email') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" name="email" class="form-control" placeholder="{{ __('auth_ui.email_placeholder') }}" value="{{ old('email') }}" required autofocus>
                    </div>
                    <button type="submit" class="btn-primary-lbt mb-3">
                        <i class="bi bi-envelope-arrow-up me-2"></i>{{ __('auth_ui.send_reset_link') }}
                    </button>
                </form>
